...
Cross-platform detection tools (Bash + Windows Batch) to identify indicators of compromise from the September 2025 npm supply chain attacks, including the Shai-Hulud self-replicating worm and the chalk/debug crypto theft attack. This comprehensive detector covers 571+ compromised package versions across multiple attack campaigns, providing protection against the most severe JavaScript supply chain attacks to date. The self-replicating worm represents one of the most severe JavaScript supply chain attacks to date, surpassing previous incidents. The scripts currently detect 571+ confirmed compromised package versions, including popular packages like `@ctrl/tinycolor` with 2 million weekly downloads and `chalk` with over 2 billion weekly downloads.
...
**✨ New in v3.0.0**: The Windows Batch script now includes all advanced features:
...
- **2025-09-19 v3.0.0**: **Ultimate Enhancement Release** - Complete feature parity with multi-attack coverage
  - **Multi-Attack Coverage**: Added 26 packages from Chalk/Debug crypto theft attack (571+ total)
  ...
  - **Crypto Theft Detection**: Covers cryptocurrency wallet replacement patterns, XMLHttpRequest hijacking
